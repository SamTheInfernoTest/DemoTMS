...
export function UserContextProvider({ children }) {
    ...
    function loginTheUser(userType , uid, name, profileImage, standards, refreshToken, token, bgImage, moto){
        ...
        setUid(uid)
        setName(name)        
        setProfileImage(profileImage ? `${'http://localhost:2000'}${profileImage}` : '/user.svg')
        setStandards(standards)
        setToken(token)
        setRefreshToken(refreshToken)
        setBgImage(bgImage ? `${'http://localhost:2000'}${bgImage}` : "https://picsum.photos/1200/400")
        setMoto(moto)
        
        sessionStorage.setItem('userType', userType)
        sessionStorage.setItem('uid', uid)
        sessionStorage.setItem('name', name)
        sessionStorage.setItem('profileImage', profileImage ? `${'http://localhost:2000'}${profileImage}` : '/user.svg')
        sessionStorage.setItem('standards', JSON.stringify(standards))
        sessionStorage.setItem('refreshToken', refreshToken)
        sessionStorage.setItem('bgImage', bgImage ? `${'http://localhost:2000'}${bgImage}` : "https://picsum.photos/1200/400")
